...
const registerAutocomplete = (commands, settings) => {
  const completion = omelette(`${settings.invocation} <argv0> <argv1>`);
  // This is your logic
  ...
  completion.on('argv1', ({ reply }) => {
    const subCommands = commands.reduce((subs, command) => {
      const cmd = Object.keys(command)[0];
      const commandEntity = command[cmd];
      if (!commandEntity['sub-commands']) return subs;
      return subs.concat(
        commandEntity['sub-commands'].reduce((holder, sub) => {
          return holder.concat(Object.keys(sub));
        }, [])
      );
    }, []);
    reply(subCommands);
